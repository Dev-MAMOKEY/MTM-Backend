...
/**
 * 이미지 생성 공급자가 선택된 환경에서 기준 이미지 생성·조회 요청을 처리하는 API.
 */
@Tag(name = "기준 이미지", description = "기준 이미지 생성 · 목록 조회")
@RestController
...
public class BaseImageController {
    ...
    @PostMapping("/photos/{photoId}/base-image")
    ...
    @PostMapping("/photos/{photoId}/base-image/regenerate")
    ...
    /**
     * 로그인한 회원이 만든 기준 이미지 목록을 조회한다.
     * 착용 화면 진입 시 baseImageId를 고를 목록(예: 마지막에 쓰던 사진)으로 쓰인다.
     */
    @Operation(
            summary = "내 기준 이미지 목록 조회",
    ...
    public ResponseEntity<RsData<List<BaseImageResponse>>> getMyBaseImages(
            @AuthenticationPrincipal Long memberId
    ) {
        return ResponseEntity.ok(
                RsData.success(baseImageService.getMyBaseImages(memberId))
        );
    }
}
