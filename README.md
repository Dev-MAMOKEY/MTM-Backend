...
> 담당자는 아직 배정되지 않았다. 배정되면 아래를 그대로 적용한다.

- **Phase 0은 회의로 진행한다.** `ImageGenerationGateway` / `ImageStorage` 인터페이스 시그니처는 화면 공유로 그 자리에서 합의하고 커밋한다. 셋이 유일하게 동시에 같은 코드를 보는 순간이다.
- **인터페이스는 동결한다.** Phase 0 이후 시그니처를 바꾸고 싶으면 혼자 고치지 말고 이슈 코멘트나 PR로 먼저 제안해 영향받는 담당자와 합의한 뒤 바꾼다.
- **`global/**`는 1차 작성자를 정한다.** Phase 0에서 `RsData`, `CustomException`/`ErrorCode`, `SecurityConfig` 기본틀을 먼저 짠 사람이 소유자가 되고, 구조를 바꾸는 PR은 그 사람 리뷰를 받는다.
- **B↔C 접점(#7, #8)마다 통합 체크포인트를 둔다.** 각자 Fake 구현체로 개발을 마쳤다고 끝난 게 아니라, 실제 구현체로 바꿔 함께 돌려보는 세션을 완료 조건에 포함한다.
- **블로커는 그날 안에 공유한다.** 크리티컬 패스(`#4→#6→#7→#8→#9→#10→#11`)를 한 사람이 잇달아 맡는 구조라, 막히면 하루 그대로 밀린다. 매일 짧게(비동기 가능) "막힌 거 있음/없음"만 공유한다.
- **진행 상태는 라벨로 드러낸다.** 기존 `ready-for-agent` / `ready-for-human` / `blocked`에 `in-progress`, `in-review`를 추가하고, 착수·리뷰 전환 시 라벨과 assignee를 바로 갱신한다.
- **PR은 작게, 자주 올린다.** 이슈 하나를 다 끝내고 한 번에 올리지 않는다. 특히 `global/**`·인터페이스를 건드리는 PR은 더 잘게 쪼갠다.
- **ADR 재평가는 캘린더에 잡는 실제 회의다.** `#9` 완료 예상일에 맞춰 미리 일정을 잡는다 — 첫 착용 이미지를 함께 보고 생성 합성/오려붙이기 전환 여부를 결정하는 자리다.
...
주제가 아니라 **패키지 경계(도메인 vs 인프라)**로 나눈다 — 담당자끼리 같은 파일을 동시에 건드릴 일이 없도록.

| 담당 | 소유 패키지 | 이슈 |
|---|---|---|
| A. 회원·제품 도메인 | `domain/member`, `domain/product`, `global/security` | #4 → #3 → #5 |
| B. 인프라 어댑터 | `infra/storage`, `infra/imagegen`, 배포 설정 | 인터페이스 설계(공동) → #7(Gemini 구현체) → #8(프롬프트 조립기) → #12(조건부) → AWS 배포 |
| C. 이미지 도메인 파이프라인 | `domain/photo`, `domain/worn` | #6 → #7(상위) → #8(서비스) → #9 → #10 → #11 |

- B는 `domain/**`을 수정하지 않는다. C는 `ImageGenerationGateway` 뒤의 `FakeImageGateway`로 먼저 개발해 B의 실제 구현을 기다리지 않는다.
- A/B/C가 유일하게 마주치는 접점은 `ImageGenerationGateway` / `ImageStorage` **인터페이스 시그니처**다. **Phase 0에서 확정한 뒤 바꾸지 않는다** — 바뀌면 B(구현체)와 C(도메인)를 동시에 다시 손대야 한다.

> ⚠️ **#10, #11 동시 진행 금지.** 둘 다 #9에만 의존해 병렬 가능해 보이지만, #11(기준 이미지 재생성)은 착용 이미지를 지우기 위해 `domain/photo`뿐 아니라 `domain/worn`도 건드린다. #10도 `domain/worn`을 건드리므로 두 사람이 동시에 하면 같은 파일이 겹친다. #9 작업 중에 `WornImageRepository`에 삭제 메서드(예: `deleteAllByBaseImageId`)를 미리 만들어 두고, #10 → #11 순서로 한 사람(C)이 이어서 처리한다.

### 진행 순서

```
Phase 0  스캐폴딩 · ERD 확정 · 인터페이스 2개(ImageStorage, ImageGenerationGateway) 확정 · API 스펙 프론트 공유 · 키 발급
Phase 1  A:#4                B:인터페이스 + Fake/Local 구현체   C:#6(Fake 기반, 테스트 먼저)
Phase 2  A:#3                B:#7 Gemini 구현체                 C:#7 상위
Phase 3  A:#5                B:#8 프롬프트 조립기                C:#8 서비스 → #9 → ⚠️ ADR 0001 재평가(전원 참여)
Phase 4  A:C 코드리뷰·테스트 지원   B:#12(조건부) + AWS 배포     C:#10 → #11 (순서 지킴)
